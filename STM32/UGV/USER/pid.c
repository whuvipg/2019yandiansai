...
void PID_InitALL(void)                //初始化位置环，速度环参数
{
	...
	PID_Init(&chassismotor_1);
 	PID_SetParam(&chassismotor_1 , 15 , 0.5 , 2 , 2000 , 10000); //40
	PID_Init(&chassismotor_2);
 	PID_SetParam(&chassismotor_2 , 15 , 0.5 , 2 , 2000 , 10000);		            
	PID_Init(&chassismotor_3);
 	PID_SetParam(&chassismotor_3 , 15 , 0.5 , 2 , 2000 , 10000);		             
	PID_Init(&chassismotor_4);
 	PID_SetParam(&chassismotor_4 , 15 , 0.5 , 2 , 2000 , 10000);		
}
